...
plt.tight_layout()
plt.savefig('presentation_images/01_target_distribution.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
...
for i, col in enumerate(key_numerical):
    ...
    axes[i].set_title(f'{col.replace("_", " ").title()} vs Loan Status', fontsize=12, fontweight='bold')
    axes[i].set_xlabel('Loan Status', fontsize=10)
    axes[i].set_ylabel(col.replace("_", " ").title(), fontsize=10)

plt.tight_layout()
plt.savefig('presentation_images/02_boxplots_key_features.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
...
plt.tight_layout()
plt.savefig('presentation_images/03_correlation_heatmap.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
...
plt.tight_layout()
plt.savefig('presentation_images/04_categorical_features.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
...
quality_data = {
    'Metric': ['Missing Values', 'Duplicate Rows', 'Negative Values', 'Data Consistency'],
...
table = ax.table(cellText=quality_df.values, colLabels=quality_df.columns,
...
plt.savefig('presentation_images/05_data_quality.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
...
for i, step in enumerate(steps):
    ...
    rect = plt.Rectangle((x_pos - box_width/2, y_pos - box_height/2), 
                        ...
                        linewidth=2, edgecolor='#2E86AB', facecolor='#A8DADC')
    ax.add_patch(rect)
    ax.text(x_pos, y_pos, step, ha='center', va='center', 
           fontsize=10, fontweight='bold')
    
    # Draw arrow
    if i < len(steps) - 1:
        ax.arrow(x_pos + box_width/2, y_pos, 0.5, 0, 
                head_width=0.1, head_length=0.15, fc='#2E86AB', ec='#2E86AB')

ax.set_xlim(0, 14)
ax.set_ylim(0, 1)
ax.set_title('Data Preprocessing Pipeline', fontsize=16, fontweight='bold', pad=20)
plt.savefig('presentation_images/06_preprocessing_pipeline.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
